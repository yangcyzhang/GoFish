...
public class FishingSpotDao_Impl(
  ...
  init {
    ...
    this.__insertAdapterOfFishingSpot = object : EntityInsertAdapter<FishingSpot>() {
      protected override fun createQuery(): String =
          "INSERT OR REPLACE INTO `fishing_spots` (`id`,`name`,`address`,`latitude`,`longitude`,`iconColor`,`iconStyle`,`arrivalTime`,`imageUrl`,`fishSpecies`,`bait`,`fee`,`notes`,`isSynced`) VALUES (nullif(?, 0),?,?,?,?,?,?,?,?,?,?,?,?,?)"
  ...
  public override suspend fun insertFishingSpot(spot: FishingSpot): Unit = performSuspending(__db,
      false, true) { _connection ->
    __insertAdapterOfFishingSpot.insert(_connection, spot)
  }

  public override suspend fun deleteFishingSpot(spot: FishingSpot): Unit = performSuspending(__db,
      false, true) { _connection ->
  ...
  public override fun getAllFishingSpots(): Flow<List<FishingSpot>> {
    ...
    return createFlow(__db, false, arrayOf("fishing_spots")) { _connection ->
      ...
      try {
        ...
        val _cursorIndexOfIconColor: Int = getColumnIndexOrThrow(_stmt, "iconColor")
        val _cursorIndexOfIconStyle: Int = getColumnIndexOrThrow(_stmt, "iconStyle")
        val _cursorIndexOfArrivalTime: Int = getColumnIndexOrThrow(_stmt, "arrivalTime")
        val _cursorIndexOfImageUrl: Int = getColumnIndexOrThrow(_stmt, "imageUrl")
        val _cursorIndexOfFishSpecies: Int = getColumnIndexOrThrow(_stmt, "fishSpecies")
        val _cursorIndexOfBait: Int = getColumnIndexOrThrow(_stmt, "bait")
        val _cursorIndexOfFee: Int = getColumnIndexOrThrow(_stmt, "fee")
        val _cursorIndexOfNotes: Int = getColumnIndexOrThrow(_stmt, "notes")
        val _cursorIndexOfIsSynced: Int = getColumnIndexOrThrow(_stmt, "isSynced")
        ...
        while (_stmt.step()) {
          ...
          _tmpIconColor = _stmt.getText(_cursorIndexOfIconColor)
          val _tmpIconStyle: String
          _tmpIconStyle = _stmt.getText(_cursorIndexOfIconStyle)
          val _tmpArrivalTime: String
          _tmpArrivalTime = _stmt.getText(_cursorIndexOfArrivalTime)
          ...
          _tmpFishSpecies = _stmt.getText(_cursorIndexOfFishSpecies)
          val _tmpBait: String
          _tmpBait = _stmt.getText(_cursorIndexOfBait)
          val _tmpFee: String
          _tmpFee = _stmt.getText(_cursorIndexOfFee)
          val _tmpNotes: String
          _tmpNotes = _stmt.getText(_cursorIndexOfNotes)
          val _tmpIsSynced: Boolean
          val _tmp: Int
          _tmp = _stmt.getLong(_cursorIndexOfIsSynced).toInt()
          _tmpIsSynced = _tmp != 0
          _item =
              FishingSpot(_tmpId,_tmpName,_tmpAddress,_tmpLatitude,_tmpLongitude,_tmpIconColor,_tmpIconStyle,_tmpArrivalTime,_tmpImageUrl,_tmpFishSpecies,_tmpBait,_tmpFee,_tmpNotes,_tmpIsSynced)
          _result.add(_item)
        }
        _result
      } finally {
